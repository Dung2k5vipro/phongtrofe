...
export default function XemPhongPage() {
  ...
  const fetchPhong = async () => {
    try {
      const result = await phongService.getPublicPhong({
        limit: 100,
      ...
      toast.error(getErrorMessage(error, 'Co loi xay ra khi tai danh sach phong'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchToaNha = async () => {
    try {
      const result = await toaNhaService.getPublicToaNha();
      if (result.success && result.data) {
