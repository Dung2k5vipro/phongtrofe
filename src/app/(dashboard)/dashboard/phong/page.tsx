...
export default function PhongPage() {
  ...
  const fetchPhong = async (forceRefresh = false) => {
    try {
      ...
      const resultPhong = await phongService.getAllPhong({
        limit: 100,
        toaNha: selectedToaNha && selectedToaNha !== 'all' ? selectedToaNha : undefined,
        trangThai: selectedTrangThai && selectedTrangThai !== 'all' ? selectedTrangThai : undefined,
      });
      const phongData = resultPhong.success && resultPhong.data ? resultPhong.data : [];
      setPhongList(phongData);

      const resultToaNha = await toaNhaService.getAllToaNha({ limit: 100 });
      const toaNhaData = resultToaNha.success && resultToaNha.data ? resultToaNha.data : [];
      ...
      toast.error(getErrorMessage(error, 'Khong the tai danh sach phong'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchToaNha = async () => {
    try {
      const result = await toaNhaService.getAllToaNha({ limit: 100 });
      if (result.success && result.data) {
        setToaNhaList(result.data);
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Khong the tai danh sach toa nha'));
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      const result = await phongService.deletePhong(id);

      if (result.success) {
        cache.clearCache();
        setPhongList((prev) => prev.filter((phong) => phong._id !== id));
        toast.success(result.message || 'Xoa phong thanh cong');
      } else {
        toast.error(result.message || 'Co loi xay ra khi xoa phong');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi xoa phong'));
    }
  };
...
function PhongForm({ 
  ...
  const getToaNhaId = (toaNha: string | { _id?: string } | null | undefined) => {
    if (!toaNha) {
      return '';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = phong
        ? await phongService.updatePhong(phong._id!, formData)
        : await phongService.createPhong(formData as any);
