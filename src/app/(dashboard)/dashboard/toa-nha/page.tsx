...
export default function ToaNhaPage() {
  ...
  const handleDelete = async (id: string) => {
    try {
      const result = await toaNhaService.deleteToaNha(id);

      if (result.success) {
        cache.clearCache();
        setToaNhaList((prev) => prev.filter((toaNha) => toaNha._id !== id));
        toast.success(result.message || 'Xoa toa nha thanh cong');
      } else {
        toast.error(result.message || 'Co loi xay ra khi xoa toa nha');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi xoa toa nha'));
    }
  };
...
function ToaNhaForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = toaNha
        ? await toaNhaService.updateToaNha(toaNha._id!, submitData)
        : await toaNhaService.createToaNha(submitData);
