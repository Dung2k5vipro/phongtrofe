...
export default function SuCoPage() {
  ...
  const fetchData = async (forceRefresh = false) => {
    try {
      ...
      const [suCoResult, phongResult, khachThueResult, hopDongResult] = await Promise.all([
        suCoService.getAllSuCo({ limit: 100 }),
        phongService.getAllPhong({ limit: 100 }),
        khachThueService.getAllKhachThue({ limit: 100 }),
        hopDongService.getAllHopDong({ limit: 100 }),
      ]);

      const suCos = suCoResult.success && suCoResult.data ? suCoResult.data : [];
      ...
      toast.error(getErrorMessage(error, 'Khong the tai du lieu su co'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      const result = await suCoService.deleteSuCo(id);

      if (result.success) {
        cache.clearCache();
        setSuCoList((prev) => prev.filter((suCo) => suCo._id !== id));
        toast.success(result.message || 'Xoa su co thanh cong');
      } else {
        toast.error(result.message || 'Khong the xoa su co');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi xoa su co'));
    }
  };

  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      const result = await suCoService.updateSuCo(id, {
        trangThai: newStatus as any,
      });

      if (result.success) {
        cache.clearCache();
        setSuCoList((prev) => prev.map((item) => (item._id === id ? { ...item, trangThai: newStatus as any } : item)));
        toast.success(result.message || 'Cap nhat trang thai thanh cong');
      } else {
        toast.error(result.message || 'Khong the cap nhat trang thai');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi cap nhat trang thai'));
    }
  };
...
function SuCoForm({ 
  ...
  const getPhongId = (value: string | Phong | undefined) => {
    if (!value) return '';
    return typeof value === 'string' ? value : value._id || '';
  };

  const getKhachThueId = (value: string | KhachThue | undefined) => {
    if (!value) return '';
    return typeof value === 'string' ? value : value._id || '';
  };

  const [formData, setFormData] = useState({
    phong: getPhongId(suCo?.phong),
    khachThue: getKhachThueId(suCo?.khachThue),
    tieuDe: suCo?.tieuDe || '',
    moTa: suCo?.moTa || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = suCo
        ? await suCoService.updateSuCo(suCo._id!, submitData as any)
        : await suCoService.createSuCo(submitData as any);

      if (result.success) {
        toast.success(result.message || (suCo ? 'Cap nhat su co thanh cong' : 'Tao su co thanh cong'));
        onSuccess();
      } else {
        toast.error(result.message || 'Khong the luu su co');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi luu su co'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handlePhongChange = async (phongId: string) => {
    ...
    if (phong) {
      ...
      const hopDongHoatDong = hopDongList.find(hd => 
        (typeof hd.phong === 'object' ? hd.phong?._id : hd.phong) === phongId && hd.trangThai === 'hoatDong'
      );
      
      if (hopDongHoatDong && hopDongHoatDong.nguoiDaiDien) {
        // Lấy người đại diện làm khách thuê chính
        const nguoiDaiDienId = typeof hopDongHoatDong.nguoiDaiDien === 'object' ? hopDongHoatDong.nguoiDaiDien?._id || '' : hopDongHoatDong.nguoiDaiDien;
    ...
    <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
      ...
      <DialogFooter className="flex-col sm:flex-row gap-2">
        ...
        <Button type="submit" size="sm" disabled={submitting} className="w-full sm:w-auto">
          {submitting ? 'Đang xử lý...' : (suCo ? 'Cập nhật' : 'Báo cáo')}
        </Button>
      </DialogFooter>
    </form>
  );
