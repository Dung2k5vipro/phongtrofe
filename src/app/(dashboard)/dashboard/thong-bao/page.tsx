...
import { ThongBao, ToaNha, Phong, KhachThue, NguoiDung } from '@/types';
...
export default function ThongBaoPage() {
  ...
  const fetchData = async (forceRefresh = false) => {
    try {
      ...
      const [thongBaoResult, toaNhaResult, phongResult, khachThueResult] = await Promise.all([
        thongBaoService.getAllThongBao({ limit: 100 }),
        toaNhaService.getAllToaNha({ limit: 100 }),
        phongService.getAllPhong({ limit: 100 }),
        khachThueService.getAllKhachThue({ limit: 100 }),
      ]);

      const thongBaos = thongBaoResult.success && thongBaoResult.data ? thongBaoResult.data : [];
      const toaNhas = toaNhaResult.success && toaNhaResult.data ? toaNhaResult.data : [];
      ...
      toast.error(getErrorMessage(error, 'Khong the tai du lieu thong bao'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const getToaNhaName = (toaNhaValue?: string | ToaNha) => {
    if (!toaNhaValue) return 'Tat ca toa nha';
    if (typeof toaNhaValue === 'object') {
      return toaNhaValue.tenToaNha || 'Khong xac dinh';
    }

    const toaNha = toaNhaList.find(tn => tn._id === toaNhaValue);
    return toaNha?.tenToaNha || 'Khong xac dinh';
  };

  const getPhongNames = (phongIds?: Array<string | Phong>) => {
    if (!phongIds || phongIds.length === 0) return 'Tat ca phong';
    const phongNames = phongIds.map(id => {
      if (typeof id === 'object') {
        return id.maPhong || 'Khong xac dinh';
      }

      const phong = phongList.find(p => p._id === id);
      return phong?.maPhong || 'Khong xac dinh';
    });
    return phongNames.join(', ');
  };

  const getKhachThueNames = (khachThueIds?: Array<string | KhachThue | NguoiDung>) => {
    if (!khachThueIds || khachThueIds.length === 0) return 'Tat ca khach thue';
    const khachThueNames = khachThueIds.map(id => {
      if (typeof id === 'object') {
        return ('hoTen' in id ? id.hoTen : id.ten) || 'Khong xac dinh';
      }

      const khachThue = khachThueList.find(k => k._id === id);
      return khachThue?.hoTen || 'Khong xac dinh';
    });
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Ban co chac chan muon xoa thong bao nay?')) {
      try {
        const result = await thongBaoService.deleteThongBao(id);

        if (result.success) {
          cache.clearCache();
          setThongBaoList((prev) => prev.filter((thongBao) => thongBao._id !== id));
          toast.success(result.message || 'Xoa thong bao thanh cong');
        } else {
          toast.error(result.message || 'Khong the xoa thong bao');
        }
      } catch (error) {
        toast.error(getErrorMessage(error, 'Co loi xay ra khi xoa thong bao'));
      }
    }
  };
              ...
              <TableBody>
                {filteredThongBao.map((thongBao) => (
                  <TableRow key={thongBao._id}>
                    ...
                    <TableCell>
                      <Badge variant={(thongBao.daDoc?.length || 0) > 0 ? "default" : "secondary"}>
                        {(thongBao.daDoc?.length || 0) > 0 ? 'Đã đọc' : 'Chưa đọc'}
                      </Badge>
                    </TableCell>
            ...
            return (
              <Card key={thongBao._id} className="p-4">
                <div className="space-y-3">
                  ...
                  {/* Read status */}
                  <div className="border-t pt-2">
                    <Badge variant={(thongBao.daDoc?.length || 0) > 0 ? "default" : "secondary"} className="text-xs">
                      {(thongBao.daDoc?.length || 0) > 0 ? 'Đã đọc' : 'Chưa đọc'}
                    </Badge>
                  </div>
...
function ThongBaoForm({ 
  ...
  const [formData, setFormData] = useState({
    ...
    nguoiNhan: (thongBao?.nguoiNhan || []).map((item) => getId(item as any)).filter(Boolean),
    phong: (thongBao?.phong || []).map((item) => getId(item as any)).filter(Boolean),
    toaNha: getId(thongBao?.toaNha as any),
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = thongBao
        ? await thongBaoService.updateThongBao(thongBao._id!, formData as any)
        : await thongBaoService.createThongBao(formData as any);

      if (result.success) {
        toast.success(result.message || (thongBao ? 'Cap nhat thong bao thanh cong' : 'Tao thong bao thanh cong'));
        onSuccess();
      } else {
        toast.error(result.message || 'Khong the luu thong bao');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi luu thong bao'));
    } finally {
      setSubmitting(false);
    }
  };
