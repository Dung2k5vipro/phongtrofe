...
const getPhongName = (phongId: string | Phong, phongList: Phong[]) => {
  const phong = resolveEntity(phongList, phongId);
  return safeText(phong?.maPhong);
};

const getKhachThueName = (khachThueId: string | KhachThue, khachThueList: KhachThue[]) => {
  const khachThue = resolveEntity(khachThueList, khachThueId);
  return safeText(khachThue?.hoTen);
};

const getHopDongName = (hopDongRef: string | HopDong, hopDongList: HopDong[]) => {
  const hopDong = resolveEntity(hopDongList, hopDongRef);
  return safeText(hopDong?.maHopDong);
};
...
export default function HoaDonPage() {
  ...
  const fetchData = async (forceRefresh = false) => {
    try {
      ...
      toast.error(getErrorMessage(error, 'Khong the tai du lieu hoa don'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (result.success) {
        ...
        toast.success(result.message || 'Hoa don da duoc xoa thanh cong');
      } else {
        toast.error(result.message || 'Khong the xoa hoa don');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi xoa hoa don'));
    }
  };
  ...
  const handleDeleteMultiple = async (ids: string[]) => {
    ...
    try {
      const results = await Promise.all(ids.map((id) => hoaDonService.deleteHoaDon(id)));
      const failedDeletes = results.filter((result) => !result.success);

      if (failedDeletes.length === 0) {
        cache.clearCache();
        setHoaDonList((prev) => prev.filter((hoaDon) => !ids.includes(hoaDon._id!)));
        toast.success(`Da xoa ${ids.length} hoa don thanh cong`);
      } else {
        toast.error(`Khong the xoa ${failedDeletes.length} hoa don`);
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi xoa nhieu hoa don'));
    }
  };
    ...
    const invoiceHtml = `
      ...
      <body>
        ...
        <div class="invoice-info">
          ...
          <p><strong>Hợp đồng:</strong> ${getHopDongName(hoaDon.hopDong, hopDongList)}</p>
        ...
        ">
          ...
          <div style="display: flex; gap: 30px; margin-bottom: 30px;">
            <div style="flex: 1;">
              ...
              <p style="margin: 5px 0; font-size: 14px;"><strong>Hợp đồng:</strong> ${getHopDongName(hoaDon.hopDong, hopDongList)}</p>
            </div>
            <div style="flex: 1;">
  ...
  const handleAutoCreateInvoices = async () => {
    if (!confirm('Ban co chac chan muon tao hoa don tu dong cho thang nay?')) {
      return;
    }

    setIsAutoCreating(true);
    try {
      const result = await hoaDonService.autoCreateHoaDon();

      if (result.success && result.data) {
        toast.success(`Da tao ${result.data.createdInvoices} hoa don tu dong`);
        if (result.data.errors?.length > 0) {
          toast.warning(`Mot so loi xay ra: ${result.data.errors.length} loi`);
        }
        cache.clearCache();
        fetchData(true);
      } else {
        toast.error(result.message || 'Khong the tao hoa don tu dong');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi tao hoa don tu dong'));
    } finally {
            ...
            <div className="space-y-4 md:space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                <div>
                  ...
                  <p className="text-xs md:text-sm"><strong>Hợp đồng:</strong> {getHopDongName(viewingHoaDon.hopDong, hopDongList)}</p>
                </div>
                <div>
...
function PaymentForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const soTienThanhToan = Math.max(0, Number(formData.soTien) || 0);
      const requestData = {
        ...formData,
        hoaDon: hoaDon._id,
      ...
      const result = await thanhToanService.createThanhToan(requestData as any);

      if (result.success) {
        toast.success(result.message || 'Thanh toan thanh cong');

        const daThanhToanMoi = (hoaDon.daThanhToan || 0) + soTienThanhToan;
        const conLaiMoi = Math.max((hoaDon.tongTien || 0) - daThanhToanMoi, 0);

        const updatedHoaDon: HoaDon = {
          ...hoaDon,
        ...
        toast.error(result.message || 'Khong the xu ly thanh toan');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi thanh toan'));
    } finally {
      setSubmitting(false);
    }
  };
