...
export default function HopDongPage() {
  ...
  const fetchData = async (forceRefresh = false) => {
    try {
      ...
      const [hopDongResult, phongResult, khachThueResult, toaNhaResult] = await Promise.all([
  ...
  const getPhongName = (phong: string | Phong) => {
  ...
  const getPhongInfo = (phong: string | Phong) => {
    if (typeof phong === 'object' && phong?.maPhong) {
      const toaNhaRef = phong.toaNha;
      const toaNhaName =
        typeof toaNhaRef === 'object' && toaNhaRef?.tenToaNha
          ? toaNhaRef.tenToaNha
          : getToaNhaName(typeof toaNhaRef === 'string' ? toaNhaRef : undefined);

      return {
        maPhong: phong.maPhong,
        toaNha: toaNhaName || 'Không xác định'
      };
    }
  ...
  const toValidDate = (value: Date | string | undefined) => {
    if (!value) {
      return null;
    }
  ...
  const isExpiringSoon = (ngayKetThuc: Date | string | undefined) => {
    const endDate = toValidDate(ngayKetThuc);
    if (!endDate) {
      return false;
    }
  ...
  const isExpired = (ngayKetThuc: Date | string | undefined) => {
    const endDate = toValidDate(ngayKetThuc);
    if (!endDate) {
      return false;
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      const result = await hopDongService.deleteHopDong(id);

      if (result.success) {
        cache.clearCache();
        setHopDongList((prev) => prev.filter((hopDong) => hopDong._id !== id));
        toast.success(result.message || 'Đã xóa hợp đồng thành công');
      } else {
        toast.error(result.message || 'Không thể xóa hợp đồng');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Không thể xóa hợp đồng'));
    }
  };
  ...
  const handleGiaHan = async (hopDong: HopDong) => {
    ...
    if (newEndDate) {
      ...
      try {
        const result = await hopDongService.updateHopDong(hopDong._id!, {
          ngayKetThuc: newEndDate,
        } as any);

        if (result.success) {
          cache.clearCache();
          toast.success(result.message || 'Gia hạn hợp đồng thành công');
          fetchData(true);
        } else {
          toast.error(result.message || 'Không thể gia hạn hợp đồng');
        }
      } catch (error) {
        toast.error(getErrorMessage(error, 'Không thể gia hạn hợp đồng'));
      } finally {
    ...
    if (confirm('Bạn có chắc chắn muốn hủy hợp đồng này?')) {
      ...
      try {
        ...
        if (result.success) {
          ...
          toast.error(result.message || 'Không thể hủy hợp đồng');
        }
      } catch (error) {
        toast.error(getErrorMessage(error, 'Không thể hủy hợp đồng'));
      } finally {
        setActionLoading(null);
      }
    }
