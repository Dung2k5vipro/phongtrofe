...
export default function ChinhSuaHopDongPage() {
  ...
  const getEntityId = (value: string | KhachThue | null | undefined): string => {
    if (!value) {
      return '';
    }
  ...
  const fetchData = async () => {
    try {
      const [hopDongResult, phongResult, khachThueResult] = await Promise.all([
        hopDongService.getHopDongById(hopDongId),
        phongService.getAllPhong({ limit: 100 }),
        khachThueService.getAllKhachThue({ limit: 100 }),
      ]);

      const hopDongItem = hopDongResult.success ? hopDongResult.data : undefined;
      if (hopDongItem) {
        setHopDong(hopDongItem);

        setFormData({
          maHopDong: hopDongItem.maHopDong || '',
          phong: typeof hopDongItem.phong === 'object' ? (hopDongItem.phong as { _id: string })?._id || '' : (hopDongItem.phong as string) || '',
          khachThueId: hopDongItem.khachThueId?.map((kt) => getEntityId(kt)).filter(Boolean) || [],
          nguoiDaiDien:
            typeof hopDongItem.nguoiDaiDien === 'object'
              ? getEntityId(hopDongItem.nguoiDaiDien as KhachThue)
              : (hopDongItem.nguoiDaiDien as string) || '',
        ...
        toast.error(hopDongResult.message || 'Khong tim thay hop dong');
        router.push('/dashboard/hop-dong');
        return;
      }

      setPhongList(phongResult.success && phongResult.data ? phongResult.data : []);
      setKhachThueList(khachThueResult.success && khachThueResult.data ? khachThueResult.data : []);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await hopDongService.updateHopDong(hopDongId, {
        ...formData,
      ...
      if (result.success) {
        toast.success(result.message || 'Cap nhat hop dong thanh cong');
        router.push('/dashboard/hop-dong');
      } else {
        toast.error(result.message || 'Khong the cap nhat hop dong');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi cap nhat hop dong'));
    } finally {
      setSubmitting(false);
    }
  };
