...
export default function KhachThuePage() {
  ...
  const fetchKhachThue = async (forceRefresh = false) => {
    try {
      ...
      const result = await khachThueService.getAllKhachThue({
        limit: 100,
        trangThai: selectedTrangThai && selectedTrangThai !== 'all' ? selectedTrangThai : undefined,
      });

      const khachThueData = result.success && result.data ? result.data : [];
      setKhachThueList(khachThueData);
      cache.setCache({ khachThueList: khachThueData });
    } catch (error) {
      toast.error(getErrorMessage(error, 'Khong the tai danh sach khach thue'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Ban co chac chan muon xoa khach thue nay?')) {
      setActionLoading(`delete-${id}`);
      try {
        const result = await khachThueService.deleteKhachThue(id);

        if (result.success) {
          cache.clearCache();
          setKhachThueList((prev) => prev.filter((khachThue) => khachThue._id !== id));
          toast.success(result.message || 'Xoa khach thue thanh cong');
        } else {
          toast.error(result.message || 'Co loi xay ra');
        }
      } catch (error) {
        toast.error(getErrorMessage(error, 'Co loi xay ra khi xoa khach thue'));
      } finally {
...
function KhachThueForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = khachThue
        ? await khachThueService.updateKhachThue(khachThue._id!, submitData)
        : await khachThueService.createKhachThue(submitData);

      if (result.success) {
        onSuccess(result.data as KhachThue | undefined);
      } else {
        toast.error(result.message || 'Co loi xay ra');
      }
    } catch (error) {
      toast.error(getErrorMessage(error, 'Co loi xay ra khi gui form'));
    } finally {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
        <div className="relative">
          ...
          <Input
            placeholder="Tim kiem khach thue..."
            value={searchTerm}
        ...
        <Select value={selectedTrangThai} onValueChange={onTrangThaiChange}>
          ...
          <SelectContent>
            ...
            <SelectItem value="dangThue">Dang thue</SelectItem>
            <SelectItem value="daTraPhong">Da tra phong</SelectItem>
            <SelectItem value="chuaThue">Chua thue</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border">
        <table className="min-w-full text-sm">
          <thead className="bg-muted/50">
            <tr>
              <th className="px-3 py-2 text-left font-medium">Khach thue</th>
              <th className="px-3 py-2 text-left font-medium">Lien he</th>
              <th className="px-3 py-2 text-left font-medium">Trang thai</th>
              <th className="px-3 py-2 text-right font-medium">Thao tac</th>
            </tr>
          </thead>
          <tbody>
            {data.map((khachThue) => (
              <tr key={khachThue._id} className="border-t">
                <td className="px-3 py-2">
                  <div className="font-medium">{khachThue.hoTen}</div>
                  <div className="text-xs text-gray-500">{khachThue.cccd}</div>
                </td>
                <td className="px-3 py-2">
                  <div>{khachThue.soDienThoai}</div>
                  <div className="text-xs text-gray-500">{khachThue.email || 'Chua cap nhat'}</div>
                </td>
                <td className="px-3 py-2">
                  <Badge variant={khachThue.trangThai === 'dangThue' ? 'default' : 'secondary'}>
                    {khachThue.trangThai}
                  </Badge>
                </td>
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-2">
                    <Button variant="outline" size="sm" onClick={() => onEdit(khachThue)}>
                      <Edit className="h-4 w-4" />
                    </Button>
                    <DeleteConfirmPopover
                      onConfirm={() => onDelete(khachThue._id || '')}
                      title="Xoa khach thue"
                      description="Ban co chac chan muon xoa khach thue nay?"
                      loading={actionLoading === khachThue._id}
