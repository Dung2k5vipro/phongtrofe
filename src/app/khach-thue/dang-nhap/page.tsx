...
import type { TenantLoginPayload } from '@/types/auth';

export default function KhachThueDangNhapPage() {
  const router = useRouter();
  const [formData, setFormData] = useState<TenantLoginPayload>({
    soDienThoai: '',
    matKhau: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await authService.tenantLogin(formData);
      ...
      toast.error(getErrorMessage(error, 'Có lỗi xảy ra khi đăng nhập'));
    } finally {
      setIsLoading(false);
    }
  };
